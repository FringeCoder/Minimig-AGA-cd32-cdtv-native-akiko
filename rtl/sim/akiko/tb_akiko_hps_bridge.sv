...
//
// Akiko M3 HPS bridge bench.
//
// DUT  : akiko #(.NATIVE_CD32(1)) + akiko_hps_bridge wired together.
//        Tests the round trip: chip RAM -> TX engine -> command framer ->
//        bridge -> Main reads -> Main writes -> bridge -> result buffer ->
//        RX engine -> chip RAM.
// BFM  : 64 KiB sparse byte RAM behind dma_req/ack (copied from M2 with
//        the same `!dma_ack` re-arm guard) plus a tiny UIO BFM that drives
//        uio_cs / uio_wr / uio_rd / uio_din and observes uio_dout one byte
//        at a time per posedge clk strobe.
//
// Sim gotchas applied (from feedback_sim_gotchas memory):
//   1. wait_* tasks lead with @(posedge clk) so NBA writes commit.
//   2. Chip-RAM BFM re-arms with `dma_req && !dma_ack` to avoid the phantom-
//      transaction race the M2 bench burned cycles on.
...
// Akiko HPS bridge port (akiko <-> bridge)
...
// -----------------------------------------------------------------------
// CPU bus drivers (subset from M2 — only what we need to set up TX/RX)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Chip-RAM BFM (carbon-copy of M2 bench's BFM, including the !dma_ack
// re-arm guard. See feedback_sim_gotchas memory entry for why.)
// -----------------------------------------------------------------------
logic [7:0] mem [65536];
...
// -----------------------------------------------------------------------
// UIO BFM — single-byte read/write strokes that drive the bridge.
// Each stroke = one posedge clk where uio_rd or uio_wr is high (the bridge
// pulses cmd_pop / result_push the same cycle, akiko commits the NBA next
// cycle). Mirrors the hps_ext.v shape: cs is held for the duration of a
// transaction, then dropped to release the bridge (cmd_done / result_done).
// -----------------------------------------------------------------------
...
task automatic uio_end;
	...
	uio_rd <= 0;
	uio_wr <= 0;
	@(posedge clk); // let cs_d catch up so xfer_end pulses
	@(posedge clk);
endtask

task automatic uio_read_byte(output logic [7:0] b);
	// uio_dout is combinational on hps_cmd_byte (= buffer[rd_ptr]). Sample
	// it FIRST (rd_ptr is whatever the previous call's pop NBA settled to),
	// then pulse uio_rd for one cycle, then wait one more @(posedge clk)
	// so the rd_ptr++ NBA commits before the next call samples uio_dout.
	// Without that trailing wait we'd read byte N-1 on call N (off-by-one).
	b = uio_dout;
	uio_rd <= 1;
	@(posedge clk);
	uio_rd <= 0;
...
// -----------------------------------------------------------------------
// Wait helpers — leading @(posedge clk) per the M2 sim-gotchas memory.
// -----------------------------------------------------------------------
...
initial begin
	...
	// =====================================================================
	// Test A: TX framing of a 3-byte cmd 0x05 LED packet.
	// chip RAM at base+0x200 holds {0x05, 0x80, chk}. After TX completes,
	// cdrom_command_length should hit 3 and cmd_pending should rise.
	// =====================================================================
	$display("--- Test A: cmd 0x05 LED frames as 3 bytes ---");
	set_misc_base(24'h010000);
	mem[16'h0200] = 8'h05;
	mem[16'h0201] = 8'h80;
	mem[16'h0202] = 8'h7A; // checksum (0xff - 0x05 - 0x80 = 0x7A)
	...
	// =====================================================================
	// Test B: bridge reads command bytes back via uio_rd strokes.
	// =====================================================================
	...
	// =====================================================================
	// Test C: bridge writes a 4-byte response via uio_wr strokes.
	// After uio_end, hps_result_done fires and cdrom_receive_length latches 4.
	// =====================================================================
	$display("--- Test C: write response via bridge ---");
	// Move to a fresh misc_base so the RX writes land somewhere clean.
	set_misc_base(24'h020000);
	for (int i = 0; i < 16; i++) mem[16'h0000 + i] = 8'h00;
	// RX engine config — we're going to rely on it draining after uio_end.
	...
	// =====================================================================
	// Test D: RX engine DMAs the just-pushed response to chip RAM.
	// =====================================================================
	...
	// =====================================================================
	// Test E: 13-byte command (cmd 0x04 PLAY/READ, length 12 + chk).
	// cmd_pending must NOT rise until all 13 bytes are in the buffer.
	// =====================================================================
	$display("--- Test E: cmd 0x04 frames at 13 bytes ---");
	do_reset();
	bus_write_long(5'b00100, 32'hFF000000);
	set_misc_base(24'h030000);
	mem[16'h0200] = 8'h04;       // PLAY/READ opcode
	for (int i = 1; i <= 11; i++) mem[16'h0200 + i] = i[7:0];
	mem[16'h020C] = 8'hAA;       // arbitrary checksum byte
	u_dut.g_cd.cdrom_command_length = 6'd0;
	u_dut.g_cd.cdcomtxinx           = 8'd0;
	set_config(CFG_TXD);
	// Issue rxcmp first as 6 to test that cmd_pending stays low partway.
	write_txcmp(8'd6);
	repeat (60) @(posedge clk);
	check8 ("E1.cmdlen_partial", 8'd6, {2'h0, u_dut.g_cd.cdrom_command_length});
	check_bit("E1.pending_low",  1'b0, hps_cmd_pending);
	// Bump txcmp to 13, framer should latch pending after the last byte.
	write_txcmp(8'd13);
	wait_cmd_pending(200, cyc);
	...
	// =====================================================================
	// Test F: cmd 0x07 INFO (length 1, total 2 bytes incl checksum).
	// Smallest framable command — exercises the lookup at op==7.
	// =====================================================================
	...
	mem[16'h0200] = 8'h07;
	mem[16'h0201] = 8'hF8; // 0xff - 0x07 = 0xF8
	u_dut.g_cd.cdrom_command_length = 6'd0;
	u_dut.g_cd.cdcomtxinx           = 8'd0;
	set_config(CFG_TXD);
	write_txcmp(8'd2);
	...
	// =====================================================================
	// Test G: back-to-back. After bridge consumes F's command, framer must
	// release for a second packet from the same TX stream.
	// =====================================================================
	...
	// Drop another 2-byte INFO into chip RAM at the next position (txinx
	// already advanced to 2; bump txcmp to 4).
	mem[16'h0202] = 8'h07;
	mem[16'h0203] = 8'hF8;
	write_txcmp(8'd4);
	wait_cmd_pending(200, cyc);
	...
	// =====================================================================
	// Test H: unknown opcode (0x0B is -1 in WinUAE table).
	// Framer should hold pending=0 until length hits 32 (buffer full).
	// =====================================================================
