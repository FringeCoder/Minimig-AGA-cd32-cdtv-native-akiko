...
wire interfaceBusy; // We'll use this to PAUSE DMA reads an WRITES until ready


// The speed was set to 28.000000 Mhz, which is actually incorrect.
...
MiSTerFloppySHUGART #(28687500, 1) db(
	...
	.o_nTrk00(_track0),
	.o_nWriteProtected(_wprot),
	.o_nDiskChange(_change),
	.o_nReady(_ready),
	
	.i_nDriveSelect0(_sel0),
	.i_nDriveSelect1(_sel1),
	.i_nDriveSelect2(_sel2),
	.i_nDriveSelect3(_sel3),
	 ...
	 if (clk7_en) begin
		...
		if (selected) begin
			...
			if (~interfaceBusy) begin
				...
				if (roll_over_edge) dsk_bit <= 1'b0;
			...
			if ((~interfaceBusy) && (~fifo_empty)) begin
				_dkwe <= 1'b0;
			end
...
assign ext_floppy_rx = dma_data;
assign ext_floppy_wr = dma_wr;
assign ext_floppy_sync = dma_data == ext_floppy_syncword;
